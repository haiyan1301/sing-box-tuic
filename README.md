...
- 安装前检查脚本依赖命令，使用系统包管理器更新索引并安装/更新依赖组件。
...
- 配置完成后优先输出可直接使用的 sing-box 客户端完整 `config.json`、终端二维码和 PNG 二维码文件；TUIC URI 仅作为兼容分享链接输出。
...
`sing-box-tuic-client.json` 是可直接用于 sing-box 客户端的完整配置，包含本地 `mixed` 入站、TUIC 出站和 route；`sing-box-tuic-client-json.png` 是该 JSON 的二维码。TUIC URI 不是统一标准，部分客户端无法识别，遇到导入失败时请使用 JSON 配置。
